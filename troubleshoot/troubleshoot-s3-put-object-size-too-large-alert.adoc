...
summary: 如果租戶嘗試執行超過 S3 大小上限 5 GiB 的非多重部分「放置物件」作業、就會觸發 S3 「放置物件太大」警示。 
---
...
[role="lead"]
如果租戶嘗試執行超過 S3 大小上限 5 GiB 的非多重部分「放置物件」作業、就會觸發 S3 「放置物件太大」警示。
...
. 告知租戶、最大放置物件大小為 5 GiB 、並將多個部分上傳用於大於 5 GiB 的物件。
. 如果應用程式已變更、請忽略警示一週。
