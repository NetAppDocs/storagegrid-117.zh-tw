...
+
** 若要將應用裝置儲存節點新增至現有的 StorageGRID 系統、請參閱的指示 link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["新增網格節點"]。
** 若要將應用裝置儲存節點部署為還原作業的一部分、請參閱說明 link:../maintain/recovering-storagegrid-appliance-storage-node.html["恢復應用裝置儲存節點"]。
...
NOTE: 如果您要將儲存節點應用裝置部署為節點複製目標、請在此停止部署程序、然後繼續link:../commonhardware/appliance-node-cloning-procedure.html["節點複製程序"]。
