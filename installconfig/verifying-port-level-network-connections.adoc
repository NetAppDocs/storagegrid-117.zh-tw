...
此外、您也可以在管理網路和用戶端網路上、以及在諸如外部NFS或DNS伺服器所使用的udp連接埠上測試連線能力。如需這些連接埠的清單、請參閱 link:../network/network-port-reference.html["網路連接埠參考"]。


NOTE: 連接埠連線表中列出的 Grid Network 連接埠僅適用於 StorageGRID 11.1.0 版。若要驗證每種節點類型的連接埠是否正確、請務必參閱StorageGRID 您的版本的《網路指南》。
